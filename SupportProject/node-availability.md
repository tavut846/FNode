...
| Node Type | Xboard Support | FNode Support | Notes |
| :--- | :---: | :---: | :--- |
...
| **Socks** | ✅ | ❌ | Supported by Xboard, but not in FNode logic. |
| **HTTP** | ✅ | ❌ | Supported by Xboard, but not in FNode logic. |
| **NaiveProxy** | ✅ | ❌ | Supported by Xboard, but not in FNode logic. |
| **Mieru** | ✅ | ❌ | Supported by Xboard, but not in FNode logic. |
...
3. **XHTTP Transport Support**:
    - **Gap**: Xboard supports the `xhttp` transport for VLESS (especially under Reality), but FNode's core logic (`core/sing/node.go`) does not have a handler for `xhttp`.
    - **Impact**: Nodes configured with `xhttp` transport in Xboard will fail to start or fall back to default TCP on FNode.
    - **Source**: `FNode/core/sing/node.go` currently only handles `tcp`, `ws`, `grpc`, and `httpupgrade`.
